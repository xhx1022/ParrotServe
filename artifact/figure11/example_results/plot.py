...
data = {"parrot": parrot, "vllm": vllm}
# names = {"parrot": "Parrot", "vllm": "基线 (vLLM)", "hf": "基线 (HuggingFace)"}
names = {"parrot": "Parrot", "vllm": "基线 (vLLM)"}
...
ax.set_xlabel("输出长度 (# 令牌数)", fontsize=26,)
ax.set_ylabel("平均延迟 (秒)", fontsize=26)
...
ax.tick_params(axis="y", labelsize=20, direction="in")
ax.tick_params(axis="x", labelsize=20, direction="in")
ax.set_xlabel("块大小 (# 令牌数)", fontsize=26)
ax.set_ylabel("平均延迟 (秒)", fontsize=26)
ax.set_xticks([_ + 0.1 for _ in x])
